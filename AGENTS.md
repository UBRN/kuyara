...
- Jev is an optional evaluation layer for the agent workflow, reached only through the user-scoped `jev` MCP described in [`docs/jev-mcp.md`](docs/jev-mcp.md): bounded candidate relevance, claim-versus-evidence checks and narrow classification, acted on only above the documented confidence gate. It never generates code, runs or replaces a test, judges a screenshot, approves a deploy, migration, credential or spend change, or filters a mandatory instruction; it is not part of the product runtime, and its absence changes nothing in the workflow. Only process text reaches it, never a diff, a source file, a secret or user data.

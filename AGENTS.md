...
- Model gender in the profile as `woman` or `man`, required, with woman listed before man. The catalog keeps its own `womens`/`mens` applicability vocabulary; a single explicit mapping converts gender to it, and catalog applicability is still not a sex field. Store the birth date as an optional device-only field, show it only as the locale-formatted date, never show an age category, and send only the band derived from its year (decided, not yet implemented; the code still stores a `womens`/`mens` clothing preference. See [ADR 0015](docs/adr/0015-gender-and-age-band-in-the-profile.md), amended by ADR 0028 and ADR 0030).
...
- Keep three primary tabs: Today at `/`, Weather at `/weather`, and Profile at `/profile`. Put the Closet and the wanted list inside Profile, and reach Settings from the Profile header rather than a tab. The English user-facing label for the garment collection is **Closet**, never "Wardrobe"; Turkish is **Gardırop**. The internal domain name, tables, route segment, types and test ids stay `wardrobe`; renaming them for terminology alone is out of scope, so this is a localization rule rather than a refactor. Keep gender prominent and required in onboarding, but place its Settings control last and do not make it prominent there. Birth date is optional and sits beside it in both places (decided, not yet implemented; see [ADR 0015](docs/adr/0015-gender-and-age-band-in-the-profile.md)). Profile, the Closet and Settings are designed: the Closet is Profile's subject, every list-shaped screen shares one row anatomy, and Settings is a native grouped list; see [ADR 0028](docs/adr/0028-the-profile-tab-and-the-list-row-anatomy.md), [ADR 0029](docs/adr/0029-the-closet-grid.md) and [ADR 0030](docs/adr/0030-settings-as-a-native-grouped-list.md) before touching any of the three.

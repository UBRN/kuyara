...
- Show ownership state only on outfit detail, never on Today. The product currently has only ownership *action* copy; the approved state strings are recorded in ADR 0026 and land with the screen that uses them.
...
- Model gender in the profile as `woman` or `man`, required, with woman listed before man. The catalog keeps its own `womens`/`mens` applicability vocabulary; a single explicit mapping converts gender to it, and catalog applicability is still not a sex field. Store dress style as required `casual`, `smart`, or `formal`; it reorders formality and excludes nothing. Store the birth date as an optional device-only field, show it only as the locale-formatted date, and keep it out of product logic. When analytics is implemented, it may derive a coarse age bucket only at emit time; neither the date nor its year may leave the device. See [ADR 0031](docs/adr/0031-dress-style-is-the-formality-signal.md).
...
- Keep three primary tabs: Today at `/`, Weather at `/weather`, and Profile at `/profile`. Put the Closet and the wanted list inside Profile, and reach Settings from the Profile header rather than a tab. The English user-facing label for the garment collection is **Closet**, never "Wardrobe"; Turkish is **Gardırop**. The internal domain name, tables, route segment, types and test ids stay `wardrobe`; renaming them for terminology alone is out of scope, so this is a localization rule rather than a refactor. Onboarding has five steps: welcome, required gender, required dress style, optional birth date, and optional location selection. Keep gender prominent there, but place the About you Settings group last and unprominent, with Gender, Dress style, then Birth date. Profile, the Closet and Settings are designed: the Closet is Profile's subject, every list-shaped screen shares one row anatomy, and Settings is a native grouped list; see [ADR 0028](docs/adr/0028-the-profile-tab-and-the-list-row-anatomy.md), [ADR 0029](docs/adr/0029-the-closet-grid.md), [ADR 0030](docs/adr/0030-settings-as-a-native-grouped-list.md), and [ADR 0031](docs/adr/0031-dress-style-is-the-formality-signal.md) before touching any of the three.
...
- Before UI, UX, theme, icon, illustration, animation, splash, or branding work, read the canonical [`docs/design/visual-identity.md`](docs/design/visual-identity.md) and treat its approved decisions as constraints. The accepted visual direction is Direction E, recorded in [ADR 0021](docs/adr/0021-direction-e-a-visual-first-design-language.md): styling-first with weather as an input, a visual-first Today whose hero is a garment composition rather than a type role, garment illustration permitted, and the weather tinting the surface the garments sit on rather than occupying its own band. It amends parts of ADR 0017 and ADR 0018; read it before either. Where each garment sits on Today's stage and how large it is drawn is [ADR 0025](docs/adr/0025-the-garment-board-composition-rule.md), specified in [`docs/design/garment-board.md`](docs/design/garment-board.md); read it before any garment board or silhouette work. The recommendation detail surface it feeds is [ADR 0026](docs/adr/0026-the-recommendation-detail-surface.md).
- Do not introduce new brand colors, fonts, icon geometry, visual metaphors, or motion styles without explicit approval, and never silently modify the approved Balanced Horizon V2 master geometry. Garment silhouettes are approved as a vocabulary by ADR 0021, and ADR 0025 approves the twenty-two drawings that cover all 27 outfit-eligible catalog types; further additions to that vocabulary still need approval.

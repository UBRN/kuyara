...
- Record only `on-device-ai`, `ai-assisted`, or `deterministic-fallback` as generation mode. Never expose provider/model identity or technical failures in Today, recommendation detail, analytics, or the durable model. Today badges say where the outfit was chosen: the on-device badge pairs the multicolor original `apple.intelligence` SF Symbol with words naming Apple Intelligence in a non-purple badge; the Worker badge pairs "Chosen with AI" with its own colorful AI symbol in the purple family. Fallback has no badge. Detail has one plain source sentence per mode with kuyara as subject. Only Settings Service providers may receive the controlled, non-secret last-check provider/model ID; its Apple Intelligence device status line shows the same symbol beside words and a status shape, and its footer carries attribution. Use the Apple Intelligence word mark only in the on-device badge and spoken label, detail source sentence, and Settings status/footer; never translate, abbreviate, or add a trademark sign. See [ADR 0034](docs/adr/0034-on-device-ai-selection-through-apple-foundation-models.md).

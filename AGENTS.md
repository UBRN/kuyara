...
- Record only `on-device-ai`, `ai-assisted`, or `deterministic-fallback` as generation mode. Never expose provider/model identity or technical failures in Today, recommendation detail, analytics, or the durable model. Today badges say where the outfit was chosen: the on-device badge pairs the system's rendering of the `apple.intelligence` SF Symbol with words naming Apple Intelligence in a non-purple badge; the Worker badge pairs "Chosen with AI" with the SF Symbol `sparkles` drawn as a vivid multicolour animated layer (violet, fuchsia, gold). Fallback has no badge. Detail has one plain source sentence per mode with kuyara as subject. Only Settings Service providers may receive the controlled, non-secret last-check provider/model ID; its Apple Intelligence device status line shows the system's rendering of the same symbol beside words and a status shape, and its footer carries attribution. Use the Apple Intelligence word mark only in the on-device badge and spoken label, detail source sentence, and Settings status/footer; never translate, abbreviate, or add a trademark sign. See [ADR 0034](docs/adr/0034-on-device-ai-selection-through-apple-foundation-models.md).
...
- New brand colors, fonts, icon geometry, visual metaphors, motion styles, or additions to ADR 0025 garment silhouettes require explicit approval. Never silently alter the Balanced Horizon V2 master geometry. Vault mockups may try new colours, fonts, icon geometry, metaphors and motion when each departure is labelled as a proposal; approval is given once, when the owner adopts a mockup.
...
- Routine UI acceptance requires theme and component suites, design-language greps, and one affected Simulator run for iOS changes. Manual VoiceOver checks are excluded from acceptance. Manually inspect focus order and the largest standard text size for direct accessibility changes, the dedicated accessibility/polish milestone, or a user request. The largest standard text size is the iOS Larger Text slider maximum with Larger Accessibility Sizes off (`UIContentSizeCategoryExtraExtraExtraLarge`); the Android equivalent is still to be researched. Accessibility text sizes above it (AX1 to AX5) are not tested for crashes, lost controls or overlap and are never a release or review finding. Report documentation/implementation conflicts.

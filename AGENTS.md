...
- Three layers. Active documents (`AGENTS.md`, [`docs/current-status.md`](docs/current-status.md), [`docs/product-decisions.md`](docs/product-decisions.md), [`docs/architecture.md`](docs/architecture.md), and the canonical `docs/design/` documents) carry only what is true today. ADRs are the decision history: superseded or amended decisions are marked, never rewritten or deleted. Git history is the record of completed implementation work; active documents do not repeat it.

...
- Weather supports a stable manual sample catalog for İstanbul, Ankara, and London plus an explicit foreground-only device-location choice. No location prompt occurs during app bootstrap or merely by opening the Weather tab. Decided and pending ([ADR 0016](adr/0016-location-in-onboarding-and-an-honest-empty-state.md), 2026-09-03): onboarding gains an explained location step, which narrows this rule rather than discarding it. Bootstrap and opening the Weather tab still prompt for nothing; a request may follow only kuyara's own localized explanation, and declining must remain a completable path.

...
- Automated JavaScript error tracking uses **PostHog Error Tracking** under [ADR 0035](adr/0035-posthog-error-tracking.md). **Session replay is an evaluation item, not an approval.** Worker operational monitoring uses the existing **Cloudflare Workers Logs**; Grafana and OpenTelemetry are deferred until a concrete alerting need justifies evaluating them, within the existing spending controls.

...
- Where the selection runs is decided in [ADR 0034](adr/0034-on-device-ai-selection-through-apple-foundation-models.md): an ordered chain of on-device Apple Foundation Models when the device reports the model available, then the Worker AI chain, then the deterministic device-local fallback. The on-device tier gets at most 6 seconds, the Worker request then gets its own 38 seconds (the refresh wait below), and an unavailable on-device answer costs no attempt, so ineligible devices and Android behave exactly as they did before. The Worker tier is not a legacy path and is never removed, weakened, or made conditional on the on-device tier.

...
- Implemented below the UI on 2026-09-08 (ADR 0015, amended 2026-09-07): the profile stores `gender` (`woman`/`man`, nullable until chosen) and optional device-only `birthDate`. One mapping in the profile application controller derives the catalog `womens`/`mens` vocabulary and converts existing picker writes to gender. The band is derived from the birth year at use, with null resolving to `adult`, and is the only age value transmitted. Phase 3 still owns the gender and birth-date controls, woman-first ordering and onboarding reset; existing screens are unchanged, and no age category is user-facing. See [ADR 0015](adr/0015-gender-and-age-band-in-the-profile.md).
...
- The profile receives one Expo Crypto UUID v4 when it is first created. It starts with null gender and birth date, system language, system appearance, and incomplete onboarding; repeated or concurrent initialization returns the same persisted row.
...
- AI receives deterministic weather requirements, precomposed option identifiers and their garment properties, clothing preference, age band, catalog version, and a local day variant. The catalog is filtered by clothing preference before composition, and AI may select only option identifiers supplied in the request.
...
Approved 2026-08-13 and narrowed on 2026-08-30. The simpler catalog-only boundary removes every Wardrobe-derived field. The strict request schema admits only the fields listed below, so forbidden fields have no representation and are rejected rather than filtered. Implemented 2026-09-08 (ADR 0015): the boundary includes optional `ageBand`, defaulting to `adult` when omitted for older-client compatibility. Mobile always sends the derived band. See [ADR 0015](adr/0015-gender-and-age-band-in-the-profile.md).
...
Approved 2026-08-13 and revised 2026-08-30 and 2026-09-01. Milestone 3 implemented persistence, coalescing, last-valid preservation, and the deterministic fallback; the local day trigger and revised cache identity are planned. The 2026-09-01 revision adds the missing-snapshot condition and names the persisted snapshot as the comparison baseline; it clarifies the existing decision rather than reversing it, so it needs no ADR. The generation-mode status surface and active AI probe were implemented in milestone 4 on 2026-08-29; see [Implemented generation-mode surface, active AI probe, and Worker rate limiting](#implemented-generation-mode-surface-active-ai-probe-and-worker-rate-limiting). Implemented 2026-09-08 (ADR 0015): gender drives the existing clothing-preference signal through the application mapping, and the age band joins the persisted baseline and regeneration triggers. The trigger is the band, not the birth date: a correction that leaves the band unchanged preserves a valid snapshot. See [ADR 0015](adr/0015-gender-and-age-band-in-the-profile.md).

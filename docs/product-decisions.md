...
- The opt-in is offered in two places and nowhere else: the Settings Notifications surface, which the user can reach at any time, and one contextual offer on Today, shown at a moment an alert would have fired under [ADR 0032](adr/0032-local-weather-alert-rules.md)'s rules. The contextual offer is made once for the life of the install: accepting it or dismissing it sets a durable `weather_alert_offer_shown` flag on the profile, and it is never shown again. On Today it is a quiet row below the outfit and above the other options, one sentence naming the rule that would have fired, with an accept action and a dismiss action of the same size and no accent fill. A refused OS permission is explained there with the Settings surface's own wording and its own way into system settings.

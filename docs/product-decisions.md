...
- **The website animates always and carries its own appearance selector** (maintainer decision, 2026-09-15). The Pages site ignores the operating system's Reduce Motion setting: the scroll-driven landing stage runs wherever the browser supports it, and no page carries a reduced-motion branch. This is a website carve-out and changes nothing in the app, which still renders the static end state under Reduced Motion. Every page also offers a System, Light and Dark appearance selector beside the language menu; the choice is remembered in the browser only, is applied before the first paint, and defaults to System.

...
- Under the rationale Today says one deterministic sentence about the rest of the dressing day, or says nothing at all. It is a projection of the snapshot and never generated prose: one whole localized sentence per closed state, chosen by key and never assembled from translated fragments, with no line drawn when no state fires or when the snapshot does not reach the end of the window.
...
- Today badges the two AI generation modes and nothing else. The on-device badge appears only when the stored mode is `on-device-ai`, so the words never advertise a tier that did not produce the result, and the AI-assisted mark accompanies `ai-assisted` alone. A settled deterministic result carries no badge: the absence of the mark is the signal, while the phase line during generation still narrates the deterministic fallback. Settings shows the stored status beside an availability row that reads the device's on-device state without calling any provider.
...
- That rule is enforced against the persisted recommendation snapshot's signals (weather snapshot identity, location key, gender-derived clothing preference, dress style, bundled catalog version, and dressing-day key), never against in-memory state, so a fresh install and the first location selection both count. The day variant remains the composition seed and part of cache identity, not the day-change detector.
- Cache identity is weather snapshot identity, clothing preference, dress style, catalog version, and day variant. Every regeneration, a new local day, an explicit refresh or the **show another outfit** action on the same one, removes the persisted snapshot's three option ids from the composed candidates. The exclusion is dropped whole when fewer than three candidates would remain, so a narrow pool repeats rather than showing fewer than three outfits: hot weather composes four options, and a refresh there returns the same three. A repeat memory reaching further back than the three outfits currently on screen is a separate decision. **Show another outfit** regenerates the recommendation only and never touches weather, which is what separates it from the pull gesture. It is AI-first and provider-neutral: a default of five AI regenerations per install per local day, counted only when a tap really reaches the AI chain, so an attempt that fails into the deterministic three still counts and a pool too narrow to ask does not. Past the allowance the action stays enabled and composes the next valid three from the same validated pool, without an AI request; the user is never shown a provider, a quota or a remaining count, and the number lives in one policy module rather than in the UI or the domain. The daily number keeps one install from spending the shared Worker ceiling in a sitting; it is not derived from that ceiling. The Worker additionally caches the shared AI result under a key derived from the requirement vector without reason codes, the sorted offered option ids, clothing preference, dress style, catalog version and day variant, because the request contains no personal data. Gender never reaches the Worker under that name.
...
- **The wait shows its phase.** While a recommendation is generated, Today shows one localized phase line on a polite live region: checking the on-device AI, asking the AI stylist, answer received, preparing outfits, or using standard suggestions. On the first generation it sits under the skeleton board; during a refresh of a shown recommendation it replaces the freshness caption, and the last valid recommendation stays visible. The standard-suggestions phase is held briefly (800 ms) before the deterministic composition runs, because that composition is synchronous and the line would otherwise never be seen. One small ambient mark breathes beside the line and is still under Reduce Motion; the text is the state. The phase never names a provider or a model and is never sent to analytics.
...
- AI provenance sits beside the recommendation it describes, never as footer metadata, and never in green. The two AI generation modes carry their own localized badge and the deterministic one carries none; the on-device badge names Apple Intelligence as a referential word mark with no Apple logo, glyph, or icon, and kuyara stays the subject of the sentence.
...
- When no weather requirement fires (a mild, calm, dry day inside the comfort band) and no other reason code applies, Today's rationale line shows one deterministic status sentence (`today.mildWeatherRationale`) instead of rendering empty; the detail reasons section stays requirement-only and is hidden. The store description promises to show why each piece is there "when the weather asks for it", not for every outfit.
...
- Settings is a native inset grouped list in system colours over kuyara's ground, with kuyara's headings outside the group and a centred version line. Notifications and AI status open their own surfaces; the AI status screen is the one place that names the provider and model behind the last check.
...
- **The website animates always and carries its own appearance selector** (maintainer decision, 2026-09-15). The Pages site ignores the operating system's Reduce Motion setting: the scroll-driven landing stage runs wherever the browser supports it, and no page carries a reduced-motion branch. The app takes the same stance: Reduce Motion is not a design constraint there either, and the one rule that remains is that an indefinite loop stops under the setting ([ADR 0020](adr/0020-rewriting-the-motion-law.md)). Every page also offers a System, Light and Dark appearance selector beside the language menu; the choice is remembered in the browser only, is applied before the first paint, and defaults to System.

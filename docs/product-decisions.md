...
- `runtimeVersion` follows the app version (`appVersion` policy), so every build of one version string shares one update runtime and an EAS Update reaches only installs of that version; installs of an earlier version receive the fix through the store. This is intended: the `fingerprint` policy is documented by Expo as experimental, hashes the version string too unless configured otherwise, and would tie the runtime to `eas.json` and `.gitignore` edits. Later versions ship only through EAS Build and EAS Submit; on iOS the maintainer's remaining manual step is Submit for Review. Red line: never ship two store builds of one version string with different native code; a native change, including the on-device AI module under `apps/mobile/modules`, always bumps the date stamp. A JS-only fix for the live version is published with `eas update --channel production` only after that version's binary is on the store.

...
- That rule is enforced against the persisted recommendation snapshot's signals (weather snapshot identity, location key, gender-derived clothing preference, dress style, bundled catalog version, and local date key), never against in-memory state, so a fresh install and the first location selection both count. The day variant remains the composition seed and part of cache identity, not the day-change detector.

...
- Gender and dress style are prominent and required in onboarding because they select the catalogue and formality order. Their controls remain in the final, deliberately unprominent Settings group. See [ADR 0031](adr/0031-dress-style-is-the-formality-signal.md).
...
- The profile receives one Expo Crypto UUID v4 when it is first created. It starts with null gender, dress style, and birth date, system language, system appearance, and incomplete onboarding; repeated or concurrent initialization returns the same persisted row.
- First launch presents a four-step accountless onboarding flow: welcome, required gender, required dress style, and optional birth date. Language and appearance default to system.
- Completing onboarding atomically stores gender, dress style, birth date, and completion state, then the local route gate opens the tab group on Today.
- Settings opens from the Profile header and is not a tab. It persists language, appearance, gender, dress style, and birth date changes immediately. The About you group stays last and deliberately unprominent. The existing localization and semantic theme providers consume the saved preferences, so successful changes update visible UI without an app reload.
...
- Weather supports live city search plus an explicit foreground-only device-location choice. No location prompt occurs during app bootstrap or merely by opening the Weather tab. Implemented ([ADR 0016](adr/0016-location-in-onboarding-and-an-honest-empty-state.md), 2026-09-08): onboarding ends with an explained, optional location step; only its explicit device-location action can lead to the system request, its city search is the decline path, and onboarding remains completable with neither. Today names a missing active location and links to the same picker.
...
- The chain is now WeatherKit primary, Open-Meteo fallback, OpenWeather fallback, then the last valid device-local weather snapshot. WeatherKit was inserted at the head of the established chain rather than replacing it, and is present only when its four Apple credentials are configured. WeatherKit is live in production and serving real traffic; see [ADR 0014](adr/0014-weatherkit-at-the-head-of-the-provider-chain.md).
...
- AI receives deterministic weather requirements, precomposed option identifiers and their garment properties, clothing preference, dress style, catalog version, and a local day variant. The catalog is filtered by clothing preference before composition, and AI may select only option identifiers supplied in the request.
...
Approved 2026-08-13, narrowed on 2026-08-30, and amended by ADR 0031 on 2026-09-08. The simpler catalog-only boundary removes every Wardrobe-derived field. The strict request schema admits only the fields listed below, so forbidden fields have no representation and are rejected rather than filtered. Optional `dressStyle` defaults to `smart` for previous-client compatibility. Mobile sends the stored dress style.
...
Approved 2026-08-13 and revised through ADR 0031 on 2026-09-08. Milestone 3 implemented persistence, coalescing, last-valid preservation, and the deterministic fallback. The 2026-09-01 revision added the missing-snapshot condition and named the persisted snapshot as the comparison baseline. Gender drives the existing clothing-preference signal through the application mapping, and dress style joins the persisted baseline and regeneration triggers. Birth date changes trigger nothing. See [ADR 0031](adr/0031-dress-style-is-the-formality-signal.md).
...
- Scoped in three milestones:
  ...
  - **N2, local weather alerts.** A deterministic alert-rule module over the existing weather snapshot and hourly data, in the style of the deterministic recommendation engine. On every app open, deterministically reschedule local notifications for upcoming threshold crossings in the fresh forecast; additionally attempt a best-effort `expo-background-task` refresh when iOS grants it. The background task is a staleness reducer, not a guarantee that a change is caught, and it stops if the user swipes the app away. Plus repeat suppression and quiet hours. The alert rules, lead time, quiet hours and repeat suppression are decided in [ADR 0032](adr/0032-local-weather-alert-rules.md): precipitation onset and an 8 °C apparent-temperature swing over the remainder of the current local day, 60 minutes ahead, silent between 22:00 and 07:00, one alert per rule per location per day. Still no server.
...
- Profile's subject is the Closet: a native large title with the Settings gear as the bar button, a Closet heading row, a rail of the eight newest owned pieces as the hero, then one inset group with a Wanted row and a Location row. No cards, no personal facts; gender and birth date stay in Settings.
- One list-row anatomy is decided once and shared by every list-shaped screen: an inset group at 16 with radius 20 (hairline outline in light, Night Layer step in dark, or the system's own for a native group), a 28 × 28 radius-7 monochrome ink-alpha leading tile with a 20 glyph, a separator starting at the text edge (56 at the default size), a secondary trailing value then a chevron that stacks above `fontScale` 1.5, and kuyara's sentence-case `bodyStrong` section headings drawn outside any group with 12 below and 24 between groups.
- The Closet is a two-column grid of the rail's tile with a native segmented owned/wanted filter, kuyara-drawn category chips (the selected chip is the screen's one accent fill), and a plus bar button. Tiles draw the photo, else the type silhouette filled with the piece's colour family, else the category glyph; the three states share one frame. The empty state reuses Profile's sentence and button and retires the copy that contradicted ADR 0005.
- Settings is a native inset grouped list in system colours over kuyara's ground. Kuyara owns the title colour, tint, ground, status inks, section headings, leading tiles and a centred version line produced from one `{version}` / `{build}` template key; nothing inside a cell is kuyara's. Notifications and AI status open their own surfaces; provider and model identity stay hidden, as the AI rules already require.

...
The palette was further extended with derived semantic status values, success, warning, and danger ink and container pairs plus a `borderDefined` neutral, approved in [ADR 0010](../adr/0010-status-colours-destructive-variant-and-defined-borders.md) and detailed in [`design-language.md`](design-language.md#law-4-one-accent-and-a-controlled-role-band). These are not new brand colors: the six approved brand hexes above and the Balanced Horizon V2 master geometry remain locked and unchanged.
...
Approved by [ADR 0021](../adr/0021-direction-e-a-visual-first-design-language.md). A closed set of flat fashion drawings represents the common garment types: 33 drawings plus eight Phase 6 additions (polo, turtleneck, blouse, bomber, leather jacket, coat, loafer, rain boot), all on one 64-unit viewBox and one optical size. Realism comes from construction, not rendering: plackets, collar stands, seams, rib bands, pockets, quilting, lapels, welts and laces, drawn as one flat shade plane, one light plane and tone lines derived from the garment's own colour inside a single ink edge. There is no gradient, no alpha and no body; on Today's primary stage each garment stands on one flat opaque contact shade, the stage colour moved in OKLCH lightness only, while the ink edge carries the outline ([`garment-board.md`](garment-board.md) section 9). Each garment is colourful, in the colours it really comes in, by the palette rule in [`design-language.md`](design-language.md#law-4-one-accent-and-a-controlled-role-band). The board outline is 1.9 pt; accessory strokes scale with their drawing, so at Today's 16 pt caption size the outline is about 1.1 pt. Below 32 pt a drawing drops its tone lines, stitches, zips and hardware and keeps its fills and construction lines. A garment with no specific drawing falls back to its structural category, so a composition degrades to six shapes rather than breaking; complete per-type artwork is not required and is not built speculatively.

...
Superseded 2026-09-04 for the light appearance. [ADR 0021](../adr/0021-direction-e-a-visual-first-design-language.md) and its amendment move the light page ground to Soft Mist on every screen and stop separation depending on a card fill step, so the 1.2:1 light assertion describes an allocation the product is leaving. The tokens landed on 2026-09-08 and the assertion was replaced: `theme.test.mjs` now records the light card step as exactly 1.085:1 rather than enforcing a step, keeps the dark ladder ordering and its 1.2:1 step, and adds the stage floors, `textPrimary` on `stage` and the condition ink (`textPrimary` composited over `stage` at 0.76 alpha) on `stage` at or above 4.5:1 in both appearances, plus `textSecondary` on `background` at or above 4.5:1. The paragraph above stays as the record of why the M6-era ordering was retired. The contrast floors for text and non-text elements are not affected by any of this.

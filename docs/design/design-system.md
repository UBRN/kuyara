...
- The on-device provenance badge pairs Apple Intelligence words with the multicolor original `apple.intelligence` SF Symbol in a non-purple badge. The Worker badge pairs “Chosen with AI” with its own colorful symbol in the purple family; `sparkles` is the interim mark. Deterministic fallback has no provenance badge. See [ADR 0034](../adr/0034-on-device-ai-selection-through-apple-foundation-models.md).
...
The garment silhouette vocabulary approved by [ADR 0021](../adr/0021-direction-e-a-visual-first-design-language.md) and [ADR 0025](../adr/0025-the-garment-board-composition-rule.md) is implemented: 33 drawings cover all 49 catalogue types, 26 of them the 41 outfit-eligible types and seven of them accessories, and the shared board renderer composes pieces by their drawn bounds using Today and detail presets. The renderer can animate an entrance from the Today preset on the spatial spring role and reports settle to its caller. `GarmentTileArtwork` also supplies the Profile rail and Closet grid's shared photo, silhouette, category glyph ladder, including unreadable-photo fallback. These tiles fit drawn bounds uniformly into a centred 60% × 61% box and retain the existing `GarmentSlotGlyph` for legacy null-type rows. Their appearance-specific colour-family fills are approved content colour for the rail and grid only (ADR 0028 section 6 and ADR 0029 section 5), not semantic theme roles; absent colour uses the neutral derived from the page ground, and multicolor uses a per-tile two-stop diagonal gradient. Today and detail boards derive their fills from the plane they sit on: a neutral base, a deeper neutral for footwear and at most one luminance-matched accent on the board that is the subject of the screen. Accessories use their own seven silhouettes on the Profile rail, Closet grid, recommendation detail and Today badges ([ADR 0025](../adr/0025-the-garment-board-composition-rule.md)); the board renderer never places them inside an outfit board.

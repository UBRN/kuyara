...
- `GarmentSlotGlyph` and `GarmentSlotTile` (`apps/mobile/src/components/ui/garment-slot-glyph.tsx`) render the four-slot garment iconography as plain Views rather than through `Icon`. See [ADR 0008](../adr/0008-expanding-the-visual-vocabulary-for-m6-1.md) for why SF Symbols, Material Symbols, and `react-native-svg` were all rejected for this case.

...
Migration numbers are assigned when a phase starts, in ship order; two phases shipped in one binary share one migration. Each adds one explicit ordered migration after the current version, with upgrade tests, realistic device-database replay and independent read-only review before shipping. Do not edit released migrations, skip a version, rebuild a table for these additive changes, use a destructive fallback, or lose existing rows. See [ADR 0037](adr/0037-daily-formality-and-style-aesthetics.md) and [ADR 0038](adr/0038-outfit-history.md).

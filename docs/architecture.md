...
Version 8 rebuilds `local_profiles` to convert the clothing preference into `gender` (`womens` to `woman`, `mens` to `man`) and adds a nullable ISO calendar-date `birth_date`, preserving every other column, dependent row and the onboarding flag; the rebuild runs a `foreign_key_check` on the transaction connection before commit, and a failed rebuild rolls back. Foreign keys are enforced on every connection, the shared one and each transaction connection alike, so `ON DELETE RESTRICT` and the one `ON DELETE CASCADE` on hourly entries are runtime guarantees; the weather data source still deletes hourly entries explicitly so that behaviour does not depend on connection setup. Two red lines follow from enforcement: a migration that rebuilds a parent table must copy the version 8 and 13 recipe (`PRAGMA defer_foreign_keys` and its reset), because dropping a parent bumps the deferred violation counter, and no code adds `INSERT OR REPLACE`, a delete on `local_profiles`, or a new delete on `weather_snapshots` without re-reading the foreign-key graph (the weather data source's existing snapshot deletes remove the hourly rows first). Orphaned rows that older builds left behind stay in place; foreign keys check the row being written, never pre-existing children. Version 9 adds the active location's `display_name` and backfills the sample cities. Version 10 adds the nullable checked `dress_style` and resets onboarding once so existing installs answer it. Version 11 adds the `weather_alert_deliveries` ledger for local weather alerts. Version 12 adds the profile-owned `analytics_consent` state, defaulting existing rows to `undecided` and constraining it to `undecided | granted | withdrawn`. Version 13 rebuilds `recommendation_snapshots` to widen the generation-mode check to `on-device-ai | ai-assisted | deterministic-fallback`, copying every existing row verbatim with foreign keys deferred for the copy so that an orphan already on the device cannot block bootstrap; see [ADR 0034](adr/0034-on-device-ai-selection-through-apple-foundation-models.md). Version 14 adds the profile's one-time weather-alert offer flag and version 15 the morning briefing's own opt-in, both additive and both starting off. Version 16 adds the nullable `weather_snapshots.daily_json` column that holds the daily outlook; every existing row keeps its weather and carries no outlook until the next refresh writes one. Migration 17 (display name and prompt gate) additively adds `local_profiles.display_name TEXT NULL` and `local_profiles.name_prompt_version INTEGER NOT NULL DEFAULT 0`, preserving existing profile rows. Migration 18 (daily styles and day choices) adds `style_aesthetics`, `morning_sheet_enabled`, and `dressing_day_choices`, preserving existing profile rows. Migration 19 (outfit history and departures) adds the nullable daily style override, outfit history, and dressing-day departures. Schema version 19 is the latest implemented version.

Schema changes for the approved phases are additive. Phase 3 adds profile `display_name TEXT NULL` (2 to 30 characters in the domain) and `name_prompt_version INTEGER NOT NULL DEFAULT 0`. Phase 4 adds `style_aesthetics TEXT NOT NULL DEFAULT '[]'`, a validated sorted JSON array of at most three closed ids, and `morning_sheet_enabled INTEGER NOT NULL DEFAULT 1`. Phase 8 adds `walkthrough_version INTEGER NOT NULL DEFAULT 0`. Each prompt gate compares the stored integer with its code version and stores that version on completion or dismissal. See [ADR 0036](adr/0036-display-name-and-one-time-prompt-gate.md).

| Phase | Table | Columns and constraints |
| --- | --- | --- |
| 4 and 5 | `dressing_day_choices` | `id TEXT` client UUID v4 primary key; `local_profile_id TEXT NOT NULL` profile foreign key; `day_key TEXT NOT NULL` as `YYYY-MM-DD` or `YYYY-MM-DD:evening`; `formality TEXT NOT NULL` checked to `casual | smart | formal`; `source TEXT NOT NULL` checked to `morning | chip | plan | random`; `created_at` and `updated_at` required; `deleted_at TEXT NULL`; `UNIQUE (local_profile_id, day_key)`; version 19 adds nullable sorted `style_aesthetics TEXT`, with null or empty resolving to Settings defaults. Morning and evening have separate rows and answers. |
| 5 | `outfit_history` | `id TEXT` client UUID v4 primary key; `local_profile_id TEXT NOT NULL` profile foreign key; bare-date `day_key TEXT NOT NULL`; `outfit_json TEXT NOT NULL` with catalog garment ids by slot, archetype id, resolved formality and `recommended | manual` source; `photo_path TEXT NULL` relative to the managed history photo segment; `worn_at`, `created_at` and `updated_at` required; `deleted_at TEXT NULL`; `UNIQUE (local_profile_id, day_key)`; re-logging a soft-deleted day revives its row. |
| 5 | `dressing_day_departures` | Version 19: UUID `id`, profile foreign key, departure-owned `day_key`, ISO `departure_at`, IANA `time_zone`, lifecycle timestamps and nullable `deleted_at`; unique `(local_profile_id, day_key)` with revive on upsert. "Now" writes no row. |

Schema version 19 is current. Every future schema change appends an explicit ordered migration with upgrade tests, realistic device-database replay and independent read-only review before shipping. Do not edit released migrations, skip a version, rebuild a table for these additive changes, use a destructive fallback, or lose existing rows. See [ADR 0037](adr/0037-daily-formality-and-style-aesthetics.md) and [ADR 0038](adr/0038-outfit-history.md).
...
SQLite weather data source        provider interface
                         ...
                         Worker HTTP provider adapter
                                  ↓
                    shared weather v2 Zod contracts
                                  ↓
...
SQLite is the durable source of truth for the active selection and the last valid snapshots. A cache is fresh through exactly 30 minutes, stale after that boundary, and still rendered while a background or manual refresh runs. A `fetchedAt` in the device's own future is tolerated up to five minutes and treated as fresh, because the Worker stamps it from Cloudflare's clock and an ordinary device clock runs seconds behind; beyond that tolerance the snapshot is rejected as invalid. Without the tolerance a device a few milliseconds slow rejects every live refresh from the deployed Worker. Failed refreshes never erase the prior valid snapshot, the daily outlook included. A refresh result is published only if it still belongs to the selected location; the repository validates time zone, source/location identity, measurements, and the shared ordered 36-hour window rule before persistence. Reads retain compatibility with already-persisted ordered same-local-day snapshots from the earlier contract.

The daily outlook is persisted as one JSON document in the schema version 16 `weather_snapshots.daily_json` column, the way a recommendation carries its context and outfits, because it is read whole, never queried by field and replaced entirely on every refresh; a child table would have bought a second cascade and a second line in the prune for nothing. The column is nullable, and null is the only way a snapshot says it carries no outlook: a row written by an earlier build, or a source that served none. The repository owns both the parse and the validation, and an unreadable or partly invalid document costs the outlook alone, never the snapshot the screen needs to render, which is why a week missing one day is dropped whole rather than drawn short. A domain snapshot therefore carries `daily` as an optional array, absent rather than empty when there is nothing to show.

The Weather screen draws the first five of the contract's seven days in its own card below the hourly one, in the ADR 0028 row anatomy: the day's condition glyph in the P1 resolver's day ink, the weekday from the `dateKey` read as a calendar date in UTC, the precipitation cell (the measured amount beside its chance, the chance alone when the provider reported no amount, and nothing at all when the day carries neither), the low and the high through the single temperature formatter, and a 4 point rail whose accent fill spans that day's range inside the week's own minimum and maximum, with a 3 point mark for the current temperature on today's row. Every row is one accessible element whose label names the weekday, the localized condition, the range and the chance, so the empty cell and the colour are never the only signals; above `fontScale` 1.5 the range stacks under the day.

The application composition uses the network-backed Worker provider adapter. It sends only normalized coordinates and the selected IANA time zone to `POST /v2/weather`, validates success and stable error bodies with the shared Zod contracts, then maps valid data into the existing provider snapshot. The request and the error body are still v1's; only the success schema is v2's, so a response that adds a key this binary does not know is still stripped and accepted, and a response with no `daily` array is an `invalid-response` rather than a snapshot missing its outlook. The mapper restores the device-local location key from the request context; profile identity, catalog identity, permission state, and accuracy remain outside the API. The deterministic in-process provider is available to focused tests only and is never the application composition.

Snapshots keep Celsius values. Presentation derives the displayed temperature unit from the device locale at format time, with no profile unit column.
...
The static year range is 1900 to 2100; the domain validates real calendar dates and rejects future dates against the local calendar. The profile owns gender, persistent dress style, birth date, and the approved optional display name, closed style aesthetics, name and walkthrough prompt gates and morning-sheet preference. Daily answers live in `dressing_day_choices`, not a profile JSON field. `ProfileApplicationController` projects `LocalProfile.clothingPreference` through one gender-to-catalog mapping. Birth date drives no product logic. The AI request builder, analytics boundary and telemetry boundary each have a test proving that `displayName` and other free-text profile fields cannot appear in output. See [ADR 0031](adr/0031-dress-style-is-the-formality-signal.md) and [ADR 0036](adr/0036-display-name-and-one-time-prompt-gate.md).
...
`recommendOutfits` is a pure function: it reads no clock, performs no I/O, and returns the same result for the same input regardless of candidate order. Recommendation cache identity is weather snapshot identity, derived clothing preference, the dressing day's resolved formality, sorted closed style aesthetics, catalog version and day variant. Generation occurs after an active-location, derived clothing-preference or dress-style change, a bundled catalog-version change, a style-aesthetics or keyed daily-formality change, the start of a new dressing day, an expired coverage window on foreground open, or an explicit user request. The existing `localDayKey` from the recommendation controller, derived through the weather domain's `wardrobeDayKey`, is the only dressing-day clock: bare local date from 04:00 to 18:00 and that date plus `:evening` through 04:00 the next morning. Morning sheet eligibility, daily formality, Plan tomorrow and outfit history all use that key; history stores its bare-date portion, so only "Wore this today" can replace a morning record with an evening look. The AI regeneration budget uses the calendar date of that key, sharing five attempts between day and evening. `wardrobeDayWindow` remains the dressing-day and weather-alert horizon. Clothing requirements use a separate coverage window that begins now or at a saved departure and ends by the departure hour in the snapshot time zone. The first four hours drive the main outfit; a short later cool spell is optional, while sustained or severe cold remains mandatory. Coverage start and end are optional persisted context fields, excluded from the coalescing identity. An unanswered evening key has its own pending state; Today opens its sheet empty. A confirmed re-ask writes its answers and starts one reserved `regenerate` refresh, and approved-trigger evaluation waits for it so its own new day type never starts a second generation. A birth date or display-name change triggers nothing. Today reads the persisted recommendation snapshot through `RecommendationApplicationProvider`; the result is tagged `on-device-ai` when the on-device tier answers, `ai-assisted` when the Worker AI call succeeds, and `deterministic-fallback` otherwise. Today never reads Wardrobe ownership state; outfit detail is the only recommendation surface that may show `owned` or `wanted`. No repository, provider DTO, runtime schema, persistence layer, network client, or global state container is introduced by Today itself; it consumes application providers mounted at the root.

Structured insight rules live in `features/weather/domain` and are exported through that feature's domain boundary for recommendation to read. Line 1 uses the existing wet, sky, apparent-temperature-extreme and wind priority chain. Line 2 takes the first matching precipitation window, sustained hot or cold run, wind window, or 8 °C cooling or warming hour. Line 2 uses the persisted outfit coverage end when available; legacy snapshots still use the dressing-day end. Results are kinds and numeric parameters, never text; Today maps each kind to a whole-sentence localization key. Thresholds remain those of the Weather screen and [ADR 0032](adr/0032-local-weather-alert-rules.md).

The first-generation runway is derived presentation state: the controller sets `showFirstGenerationOverlay` only when a recommendation for the current dressing day is absent and a generation is in flight, and hands the runway no outfit while it waits. The runway draws the neutral five-slot skeleton until the persisted snapshot for the dressing day carries the answer, then receives that chosen outfit once and dresses it; no provisional or deterministic preview is computed. The runway board fits ADR 0025's composition, trimmed to its drawn extent, into the free area with one uniform scale (`fitRunwayScale` in `compose-garment-board.ts`), and the chosen outfit is dressed in its own palette (`garment-palette.ts`), each fill made legible on the runway field by moving it in OKLCH lightness only. Other refreshes keep the inline phase line. "Skip the wait" invokes the controller's deterministic fallback early while the existing AI request continues to its 46-second bound; a later valid AI answer is stored and shown like a background refresh. The controller computes `exhausted` from the composed pool and shown set; neither flag is persisted and no Zustand store owns them. Manual catalog-piece swaps on detail live in a screen hook. The domain validator can label a swap "Unusual for this weather" without blocking it. Only "Wore this today" persists the manual set to history with source `manual`; the recommendation cache and AI request never see those swaps or Closet items.
...
Attribution (`origin.sourceId`) is the one controlled addition to the shared contract, as described above. Settings > Service providers reads it from the last valid stored weather snapshot and presents that provider's full attribution; no new API field is needed. The Apple Intelligence status there reuses the on-device availability probe and the last recommendation's stored generation mode.
...
The request follows the canonical [AI input privacy boundary](product-decisions.md#approved-ai-input-privacy-boundary). `/v1/ai/recommend` stays shape-frozen for strict installed readers. The successor `/v2/ai/recommend` accepts the v1 request plus optional sorted `styleAesthetics` (at most three closed ids) and returns the v1 picks plus optional `insightSentence`, one sentence of 1 to 90 characters without a line break. Shared schemas live in `packages/contracts`; the v2 response strips unknown keys. One Worker handler serves both routes and emits the sentence only on v2. Mobile adopts v2 for the insight, then adds aesthetics in the daily-style phase. The AI input projection does not widen for prose.

After Zod parsing, device domain validation applies [ADR 0039](adr/0039-ai-v2-route-and-the-validated-insight-sentence.md)'s decimal-digit, per-locale language-signal and banned-content checks: Turkish accepts a closed-list function word or a Turkish-specific letter, while English requires a closed-list word and rejects those letters. The approved AI input projection contains no numeric weather, so any digit is discarded with the sentence. Failure uses the deterministic localized first insight line; invalid outfit picks still reject the selection whole. Accepted prose and its locale are optional paired fields inside `recommendation_snapshots.context_json`, so older rows parse. Today shows the prose only when its stored locale matches the current language.

Under [ADR 0034](adr/0034-on-device-ai-selection-through-apple-foundation-models.md), the composition boundary returns a routed client satisfying the existing `AiClient` interface, so the controller, the mapper, persistence, and the trigger rules keep their shape. It attempts the on-device tier only when the module answers `available`, spends at most 8 seconds there, and then gives the Worker request its own 38-second wait (the Worker's 36-second walk plus 2 seconds of transport), so the whole refresh is bounded at 46 seconds and the deterministic fallback is reached only after the last provider has failed. Mobile sends that wait minus a 1-second transport margin, 37 seconds, in a private validated header, and the Worker clamps it to its own 36-second walk; the strict request body and the Worker's shared cache identity exclude it. An unavailable answer costs no attempt and no time, so Android, web, and ineligible iPhones spend nothing in that tier. Both AI tiers pass through the same validation gate, so an on-device answer the gate rejects is that tier's failure and the Worker still gets its turn; the deterministic device-local fallback keeps its place behind both and is reached by the controller's existing catch. `aiModelInputFromRequest`, `picksAreMeaningfullyDifferent`, and `meetsArchetypePrecondition` in `packages/contracts` are the one model-input projection, the one distinctness rule, and the one archetype precondition both executors use, so the field list defining what a model may see and the rules its reply is judged by exist once. The native surface is a local Expo module in Swift under `apps/mobile/modules/kuyara-on-device-ai`, declared for Apple platforms only, and `features/recommendation/data/on-device-ai-module.ts` is its only importer anywhere in the app: it exports the module or null, so application, feature, and domain code see one branch and never touch the native surface. The Service providers surface reads the module's availability, which runs no inference and consumes no quota, so the bounded Worker probe stays the only probe.
...
The mobile request carries at most 24 fully composed options, bounded by the shared contract as a transport and prompt-size limit. Shared Zod schemas enforce option and response structure; the Worker checks that every selected option was offered. The per-attempt timeout is injectable, defaulting to 7 seconds, so timeout behavior is testable without real waiting; the request deadline defaults to 36 seconds for an old client and otherwise uses the validated transmitted budget clamped from 5 to 36 seconds.

- **Contracts** (`packages/contracts/src/ai-v1.ts`, `POST /v1/ai/recommend`). Request carries `clothingPreference`, the `ClothingRequirement[]` discriminated union and a bounded `options` array of at most 24 precomposed outfits. Each option has a stable `optionId`, formality, garments and validated traits. Response success is `data.picks` with exactly three unique `{optionId, archetypeId}` pairs; each option id must come from the request and each archetype id is unique. Response error reuses the weather-v1 error shape with codes `invalid_request | not_found | method_not_allowed | ai_unavailable | internal_error | rate_limited`.

  Under [ADR 0031](adr/0031-dress-style-is-the-formality-signal.md) the strict v1 request accepts optional `dressStyle` (`casual`/`smart`/`formal`) so previous clients remain compatible. The v2 request additionally accepts optional closed sorted style-aesthetics IDs. Mobile supplies the day's resolved formality, and the Worker treats an absent style as `smart` in both prompt and shared cache key. Aesthetics rank already-valid options through a static twelve-archetype affinity table, with ties retaining composition order; deterministic fallback applies the same soft sort. Persisted recommendation context carries resolved formality and aesthetics for cache comparison; snapshots written with the retired field or with neither style field are read as `smart`. Birth date and birth year never leave the device or enter the shared cache key.

  Ordinary generation excludes the persisted snapshot's three option ids from the offered options when at least three alternatives remain; a narrower pool may repeat. A fresh install and a snapshot without three outfits exclude nothing. A confirmed “Ask the stylist again” request may select an option shown before. The Worker's shared cache key adds the sorted offered option ids and resolved style fields, so a cached response can never name an option absent from the current offer. When no other valid option remains, Today hides the outfit action and its caption.
...
- **Worker-side validation is structural and closed-set only**, not a re-implementation of `composeOutfitOptions`'s scoring or tradeoff logic: the shared schema validates response shape and unique option and archetype ids, and the Worker verifies that each picked `optionId` was offered in the request. Invalid picks collapse to `ai_unavailable`; the specific rule that failed is never exposed to the client. Outfit composition, slot completeness and mandatory-requirement satisfaction stay in mobile, where `composeOutfitOptions` and `evaluateGarmentEligibility` already live.
- **The two endpoints that call no provider** are `GET /v1/health` (generic Worker liveness) and `GET /v1/ai/ready` (configuration readiness). The active AI provider probe from [Health, readiness, and probe distinctions](#health-readiness-and-probe-distinctions) is separate: it needs real provider quota and the Settings > Service providers "Check AI status" trigger.
...
Five tables carry sync-ready identity: `local_profiles`, `wardrobe_items`, `dressing_day_choices`, `dressing_day_departures` and `outfit_history` carry `id`, `created_at`, `updated_at` and `deleted_at`, and profile-owned rows carry the `local_profile_id` link. `weather_snapshots`, `weather_hourly_entries` and `recommendation_snapshots` are caches, `weather_alert_deliveries` is a device-local delivery ledger, and `active_locations` is device selection state; none of the five is synced, none of them carries `deleted_at`, and `weather_hourly_entries` has no `id` at all. History mirror photos use the managed `kuyara/history/photos/` document path, stay in device backup and never enter AI or telemetry. That absence of sync identity is deliberate: a cache is refetched from the Worker rather than reconciled, and a delivery ledger describes one device. `analytics_consent`, `notifications_opt_in` and `birth_date` on `local_profiles` stay on the device and are never linked to an account.
...
For the daily-style and history decisions, the closed `dress_style` analytics value is the day's resolved formality, aesthetics are not measured, and `outfit_worn_logged` carries no garment identity and at most a boolean `has_photo`. The analytics schema version advances once when the first of these changes ships; display name remains forbidden free text.
...
## Approved build 15 data changes

Migration 19 is the latest implemented device schema. Migration 20 will add the fields required for a 33-colour palette, a system-picked colour, second colour and 14 two-colour or pattern choices, preserving legacy rows; it requires independent review, an upgrade test and a realistic device-database replay. Phase 6 catalog colorways provide suggested outfit colours without sending Closet data to AI. Its eight new drawings use the existing board composition ladder, and the neutral five-slot runway is presentation state, not a persisted provisional outfit.

...
The [Apple Developer Program](../AGENTS.md#apple-developer-program) status is canonical in `AGENTS.md`. The checked-in composition now serves real weather through Open-Meteo; the deterministic sample provider is a development and test source only and is never a production fallback.
...
OpenWeather joined the chain on 2026-08-29 when the `OPENWEATHER_API_KEY` Worker secret was set; the live fallback was demonstrated by making Open-Meteo unreachable and watching the same request return `origin.sourceId: "openweather"`. WeatherKit will be inserted at the head of this same chain (milestone 6, startable since the [Apple Developer Program](../AGENTS.md#apple-developer-program) pause was lifted on 2026-08-29). The mobile weather domain, its repository, the exact 30-minute freshness boundary, and last-known-good behavior did not change when the chain changed.

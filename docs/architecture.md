...
After Zod parsing, device domain validation applies [ADR 0039](adr/0039-ai-v2-route-and-the-validated-insight-sentence.md)'s snapshot-number, locale function-word and banned-content checks. Failure discards only the sentence and uses the deterministic localized first insight line; invalid outfit picks still reject the selection whole. Accepted prose is optional inside `recommendation_snapshots.context_json`, so older rows parse.

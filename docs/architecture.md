...
- **`max_tokens: 2048` is a correctness bound, not a tuning knob.** Without it both runtimes stopped at their default length and returned truncated JSON that failed `JSON.parse` or zod. It is fixed in code and deliberately absent from `Env` and `wrangler.jsonc`. The timeouts are likewise measured bounds rather than defaults, and they are one budget across the boundary: every provider that answers the 2 KB request does so within 5 seconds (Workers AI 2 to 4.5 seconds, OpenRouter 0.3 to 1.9 seconds, measured live), while a provider that does not answer stalls indefinitely, so the per-attempt timeout is 7 seconds and `index.ts` passes no override; the handler's `totalDeadlineMs` bounds the whole walk at 19 seconds (it starts no attempt that would pass the deadline and aborts the in-flight one there, so it never adds a provider call), which leaves room for a stalled first attempt, a second full attempt and a short third; and the mobile recommend client waits 20 seconds, leaving one second for transport. Red line: do not raise the per-attempt timeout back toward the deadline; a single stalled attempt then consumes the whole budget and the fallback chain never runs. The probe client's 25 seconds is separate and unchanged.

...
The static year range is 1900 to 2100; the domain validates real calendar dates and rejects future dates against the local calendar. `Profile` and its persistence record hold gender, persistent dress style, up to three closed style aesthetics, optional 2 to 30 character display name, birth date, the versioned name prompt gate, morning-sheet preference and keyed daily-formality answers. The optional name never enters AI, analytics, telemetry, providers or logs. `ProfileApplicationController` projects `LocalProfile.clothingPreference` through one gender-to-catalog mapping. Birth date drives no product logic. See [ADR 0031](adr/0031-dress-style-is-the-formality-signal.md).
...
The request follows the canonical [AI input privacy boundary](product-decisions.md#approved-ai-input-privacy-boundary). The response carries structured data. Its optional `insightSentence` is the one visible prose field: device validation enforces one sentence, at most 90 characters, the reader's language, snapshot-matching numbers and a closed banned-content list, with deterministic line-1 fallback on any failure. Other Turkish and English copy stays in localization keys. A response failing outfit validation is rejected rather than repaired into a different outfit. The installed strict v1 readers keep their exact response shape; the insight field uses a versioned successor route rather than adding a key to v1. The closed sorted aesthetics request field deploys Worker-first and remains optional for older clients.

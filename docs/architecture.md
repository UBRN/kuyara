...
Rate limiting also covers `POST /v1/ai/recommend` (per-IP, 10/60s). Both endpoints use native Cloudflare rate-limit bindings; the probe, the Workers AI attempts of `POST /v1/ai/recommend` and the two capped weather providers each count against their own named counter in the `DAILY_COUNTERS` Durable Object binding (`apps/worker/src/daily-counter.ts`). The deployed single environment keeps `durable_objects`, `ratelimits`, `vars`, and `ai` at the top level of `wrangler.jsonc`; see [ADR 0003](adr/0003-single-worker-environment.md). When a binding is absent the handlers degrade to permissive, so local development and unit tests remain usable.

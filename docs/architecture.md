...
- **Endpoints in scope for 2a**: `GET /v1/health` (generic Worker liveness) and `GET /v1/ai/ready` (configuration readiness, calls no provider). The active AI provider probe from [Health, readiness, and probe distinctions](#health-readiness-and-probe-distinctions) is the milestone 4 endpoint: it needs real provider quota and the Settings "Check AI status" trigger.

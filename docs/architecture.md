...
| --- | --- | --- |
| 4 | `dressing_day_choices` | `id TEXT` client UUID v4 primary key; `local_profile_id TEXT NOT NULL` profile foreign key; `day_key TEXT NOT NULL` as `YYYY-MM-DD` or `YYYY-MM-DD:evening`; `formality TEXT NOT NULL` checked to `casual | smart | formal`; `source TEXT NOT NULL` checked to `morning | chip | plan | random`; `created_at` and `updated_at` required; `deleted_at TEXT NULL`; `UNIQUE (local_profile_id, day_key)` |
| 5 | `outfit_history` | `id TEXT` client UUID v4 primary key; `local_profile_id TEXT NOT NULL` profile foreign key; bare-date `day_key TEXT NOT NULL`; `outfit_json TEXT NOT NULL` with catalog garment ids by slot, archetype id, resolved formality and `recommended | manual` source; `photo_path TEXT NULL` relative to the managed history photo segment; `worn_at`, `created_at` and `updated_at` required; `deleted_at TEXT NULL`; `UNIQUE (local_profile_id, day_key)` |

Schema version 18 is current. The Phase 5 `outfit_history` table below is planned and is not part of this schema. Every future schema change appends an explicit ordered migration with upgrade tests, realistic device-database replay and independent read-only review before shipping. Do not edit released migrations, skip a version, rebuild a table for these additive changes, use a destructive fallback, or lose existing rows. See [ADR 0037](adr/0037-daily-formality-and-style-aesthetics.md) and [ADR 0038](adr/0038-outfit-history.md).

...
The first-generation runway is derived presentation state: the controller sets `showFirstGenerationOverlay` only when a recommendation for the current dressing day is absent and a generation is in flight, and with it the read-only `firstGenerationPreview`, the primary outfit the deterministic composition produces for the same input, which is exactly what "Skip the wait" would save. The runway draws that preview while the AI chain runs; the preview changes no generation mode, cache key, coalescing or fallback rule and is never persisted. Other refreshes keep the inline phase line. "Skip the wait" invokes the controller's deterministic fallback early while the existing AI request continues to its 46-second bound; a later valid AI answer is stored and shown like a background refresh. The controller computes `exhausted` from the composed pool and shown set; neither flag is persisted and no Zustand store owns them. Manual catalog-piece swaps on detail live in a screen hook. The domain validator can label a swap "Unusual for this weather" without blocking it. Only "Wore this today" persists the manual set to history with source `manual`; the recommendation cache and AI request never see those swaps or Closet items.

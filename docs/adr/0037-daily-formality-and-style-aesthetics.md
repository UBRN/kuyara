...
Dismissing the sheet presents a system alert. It keeps the platform's native alert look: "Choose a day type" is the default action (bold blue text on iOS) and "Continue without choosing" is the destructive-styled action (red text on iOS); no filled buttons, no custom sheet. Continuing stores a random formality with source `random`, so the sheet does not return for that day. AI and the deterministic engine still run. The choice can be changed in the re-ask sheet. All copy is localized as whole sentences.

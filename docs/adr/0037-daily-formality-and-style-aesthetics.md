...
On the first open of a bare-date dressing day, the dismissible native sheet is offered only when no row exists for that key and the Settings switch is enabled. It asks "How do you want to dress today?" over the previous recommendation. The sheet offers Relaxed, Sharp and Formal for that day. More opens the lasting style preferences (at most three), also available in onboarding and Settings > Profile > Style preferences. Changing those preferences does not answer the day's question. A formality answer writes source `morning`. There is no second sheet at the 18:00 evening key. A chip row under Today's title upserts the bare-date row with source `chip` before 18:00 or the `:evening` row afterward. The morning look remains in history until the user explicitly records another through "Wore this today". A bottom "Plan tomorrow" row names tomorrow's weekday and date and writes tomorrow's bare-date key with source `plan` through the same sheet.

Dismissing the sheet presents a system alert. It keeps the platform's native alert look: "Choose a style" is the default action (bold blue text on iOS) and "Continue without choosing" is the destructive-styled action (red text on iOS); no filled buttons, no custom sheet. Continuing stores a random formality with source `random`, so the sheet does not return for that day and the chip shows the choice. AI and the deterministic engine still run. The choice can be changed from the chip row. All copy is localized as whole sentences.

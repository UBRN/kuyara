...
Dismissing through "Continue without choosing" presents a system alert. It keeps the platform's native alert look: "Choose a style" is the default action (bold blue text on iOS) and "Continue without choosing" is the destructive-styled action (red text on iOS); no filled buttons, no custom sheet. Continuing stores a random formality with source `random`, so the sheet does not return for that day and the chip shows the choice. AI and the deterministic engine still run. The sheet can reopen from the chip row. All copy is localized as whole sentences.

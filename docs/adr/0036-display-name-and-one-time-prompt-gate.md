...
The display name is optional, 2 to 30 characters enforced by the domain, and stored only in the local profile. Onboarding asks for it immediately after welcome and offers "Not now". Existing users see the name prompt once after the update. Migration 17 adds the name invitation's `name_prompt_version INTEGER NOT NULL DEFAULT 0` profile column. The Phase 8 walkthrough is planned for migration 20, after the planned Phase 5 migration 19, with a separate `walkthrough_version INTEGER NOT NULL DEFAULT 0` profile column. A prompt is offered when its stored value is below that prompt's code version; completion or dismissal stores that version. Each migration preserves existing profiles and answers. A later distinct prompt receives its own column, not a generic prompt-gate table.

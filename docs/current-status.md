...
- The next Goal is 2b: real AI provider adapters — an OpenRouter primary adapter and a Cloudflare Workers AI binding fallback appended to the existing ordered provider list, plus the explicitly triggered active probe. The handler, contracts, validation boundary, and endpoints from Goal 2a should not need to change to accept them.

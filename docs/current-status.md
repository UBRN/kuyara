...
2. ~~**The bottom inset.**~~ **Done, 2026-09-07.** [ADR 0027](adr/0027-the-app-shell-and-its-three-tabs.md)
   ...
   to `safeAreaInsets.bottom + spacing.md`.
...
- `garment-type-picker-screen.tsx` uses `headerHeight + spacing.lg` for the sticky-header clearance where every other screen uses `spacing.xl`. It was left alone during the bottom-inset correction and remains unresolved.
...
- Real-device VoiceOver focus order and spoken grouping remain unverified for the Weather screen's stat and forecast groups. (The Today weather card this gap was originally recorded against was deleted in M6.1; the unverified grouping moved to the Weather tab with it.) Owned by milestone 7's validation pass, not by routine tasks.
...
- Real-device VoiceOver verification of the Profile location section, the per-slot outfit rows, and the new picker screens has not been done. Owned by milestone 7's validation pass, not by routine tasks.
...
- The Closet empty-state copy promises what [ADR 0005](adr/0005-catalog-only-recommendation-candidates.md) rules out. `wardrobe.emptyBody` reads "Add the clothes you own so they are ready for future outfit choices" in English and "Sahip olduğunuz parçaları ilerideki kombin seçimleri için ekleyin" in Turkish (`apps/mobile/src/localization/messages.ts`), but the recommendation engine never reads a Closet item. Found on 2026-09-04 while designing goal 5; the goal 4 and 5 sheets replace the sentence, so the fix lands with the Closet screen rather than as a string patch.
